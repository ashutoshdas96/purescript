...
import System.FilePath (makeRelative, takeDirectory, takeDirectory, takeFileName)
...
data Module = Module ModuleIdentifier (Maybe FilePath) [ModuleElement] deriving (Show, Eq)
...
checkImportPath name _ names
  | Just name' <- stripPrefix "../" name,
    name' `S.member` names =
    Right (ModuleIdentifier name' Regular)
  ...
  where
    ...
    prelude =
      JSVariable
        ( JSAnnot
            tokenPosnEmpty
            [ CommentA tokenPosnEmpty $ "// Generated by purs bundle " ++ showVersion Paths.version,
              WhiteSpace tokenPosnEmpty "\n"
            ]
        )
        ( cList
            [ JSVarInitExpression
                (JSIdentifier sp optionsNamespace)
    ...
    moduleReference a mn =
      JSMemberSquare
        (JSIdentifier a optionsNamespace)
        JSNoAnnot
        (str mn)
        JSNoAnnot
      ...
      [ JSMethodCall
          ( JSExpressionParen
              ...
              ( JSFunctionExpression
                  ...
                  JSIdentNone
                  JSNoAnnot
                  (JSLOne (JSIdentName JSNoAnnot "exports"))
                  JSNoAnnot
